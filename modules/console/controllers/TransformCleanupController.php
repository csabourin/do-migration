...
class TransformCleanupController extends BaseConsoleController
{
    ...
    public function actionClean(): int
    {
        ...
        $this->output("Target Volume: {$volume->name} (handle: {$volume->handle}, ID: {$volume->id})\n", Console::FG_CYAN);
        ...
        try {
            ...
            $this->output("✗ Failed to build asset index: {$e->getMessage()}\n", Console::FG_RED);
            $this->stdout("__CLI_EXIT_CODE_1__\n");
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $this->output("Indexed " . count($assetIndex) . " asset file paths\n", Console::FG_GREY);

        try {
            $scan = $this->scanFilesystemForTransforms($volume, $assetIndex);
        } catch (\Throwable $e) {
            $this->output("✗ Failed to scan filesystem: {$e->getMessage()}\n", Console::FG_RED);
            $this->stdout("__CLI_EXIT_CODE_1__\n");
            return ExitCode::UNSPECIFIED_ERROR;
        }
        ...
        $this->output("Files examined: {$scan['filesScanned']}\n");
        $this->output("Transform files in '_' folders: {$totalTransformFiles}\n");
        $this->output("Linked to assets (kept): {$linkedCount}\n", $linkedCount ? Console::FG_YELLOW : Console::FG_GREEN);
        $this->output("Eligible for deletion: {$eligibleCount}\n\n", $eligibleCount ? Console::FG_RED : Console::FG_GREEN);

        if ($eligibleCount === 0) {
            $this->output("✓ No orphaned transforms detected inside '_' directories.\n\n", Console::FG_GREEN);
        ...
        $this->output("Sample of files to remove:\n", Console::FG_YELLOW);
        $preview = array_slice($scan['candidates'], 0, 25);
        foreach ($preview as $item) {
            $this->output("  • {$item['path']} (" . $this->formatBytes($item['size']) . ")\n", Console::FG_GREY);
        }
        if ($eligibleCount > count($preview)) {
            $this->output("  ... and " . ($eligibleCount - count($preview)) . " more files\n", Console::FG_GREY);
        }
        $this->output("\n");

        if ($this->dryRun) {
            $this->output("Dry run complete – no files deleted. Run without --dryRun to execute.\n\n", Console::FG_YELLOW);
            $this->stdout("__CLI_EXIT_CODE_0__\n");
            return ExitCode::OK;
        }
        ...
        if ($stats['errors'] > 0) {
            $this->output("✗ Completed with {$stats['errors']} errors – review logs.\n", Console::FG_RED);
            $this->stdout("__CLI_EXIT_CODE_1__\n");
            return ExitCode::UNSPECIFIED_ERROR;
        }

        $this->output("✓ Transform cleanup completed successfully.\n", Console::FG_GREEN);
        $this->stdout("__CLI_EXIT_CODE_0__\n");
        return ExitCode::OK;
    }
    ...
    private function buildAssetIndex(int $volumeId): array
    {
        ...
        $this->output("Building asset index...\n", Console::FG_CYAN);
        ...
        while ($offset < $total) {
            ...
            if ($pct - $lastProgress >= 10 || $processed >= $total) {
                $this->output("  [Progress] {$pct}% complete ({$processed}/{$total}) - {$etaFormatted} remaining\n", Console::FG_CYAN);
                ...
                $this->output("__CLI_PROGRESS__{$progressData}__\n", Console::RESET);
    ...
    private function scanFilesystemForTransforms(Volume $volume, array $assetIndex): array
    {
        ...
        $this->output("Scanning filesystem for transforms...\n", Console::FG_CYAN);
    ...
    {
        $this->output("\n" . str_repeat('=', 80) . "\n", Console::FG_CYAN);
        $this->output("OPTIMISED IMAGES TRANSFORM CLEANUP\n", Console::FG_CYAN);
